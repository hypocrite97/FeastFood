 ...
 * 商品servlet类
 */
@WebServlet("/CommodityClassThreeServlet")
public class CommodityClassThreeServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	CommodityClassThreeDAO tc = new CommodityClassThreeDAO();

	public CommodityClassThreeServlet() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ((action == null) || ("find".equals(action))) {
			...
			request.getRequestDispatcher("Admin/admin/index/CommodityClassThree.jsp").forward(request, response);// 跳转到CommodityClassThree.jsp
		}
		if ("del".equals(action)) {
			try {
				tc.delete(request.getParameter("CommodityClassThreeId"));
			} catch (SQLException e) {
				e.printStackTrace();
			}
			response.sendRedirect("CommodityClassThreeServlet");
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("add".equals(action)) {
			try {
				tc.insert(new CommodityClassThree(request.getParameter("CommodityClassThreeId"),
						request.getParameter("CommodityClassThreeName"),
						request.getParameter("CommodityClassThreePrice"), request.getParameter("CommodityClassTwoId"),
						null));
			} catch (SQLException e) {
				e.printStackTrace();
			}
		...
		if ("update".equals(action)) {
			try {
				tc.update(request.getParameter("CommodityClassThreeId"),
						request.getParameter("CommodityClassThreeName"),
						request.getParameter("CommodityClassThreePrice"), request.getParameter("CommodityClassTwoId"));
